...
Playlist::Playlist()
	: pVoice(nullptr),
	poCmd(nullptr), poPanCmd(nullptr), poVolumeCmd(nullptr), poStopCmd(nullptr),
...
void Playlist::Set(Snd::ID snd_id, Wave::ID wave_id, VoiceCallback *pCallback)
{
	...
	this->poVolumeCmd = new Playlist_Volume_Cmd(this->pVoice);
	this->poStopCmd = new Playlist_Stop_Cmd(this->pVoice);
	assert(this->poCmd);
}
