...
Handle::Status Voice::Start(float volume)
{
	...
	{
		HRESULT hr;
		hr = poSourceVoice->SetVolume(volume);
		assert(hr == S_OK);
...
Handle::Status Voice::SetVolume(float volume)
{
	Handle::Lock lock(this->handle);
...
Handle::Status  Voice::Pan(IXAudio2MasteringVoice* MasterVoice, float snd_pan)
{
	Handle::Lock lock(this->handle);
	...
	if (lock)
	{
		...
		poSourceVoice->GetVoiceDetails(&VoiceDetails);

		XAUDIO2_VOICE_DETAILS MasterVoiceDetails;
		MasterVoice->GetVoiceDetails(&MasterVoiceDetails);

		hr = poSourceVoice->SetOutputMatrix(NULL, VoiceDetails.InputChannels, MasterVoiceDetails.InputChannels, outputMatrix);
		assert(hr == S_OK);
