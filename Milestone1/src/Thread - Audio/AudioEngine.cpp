...
void AudioEngine::StartSound(Voice *pVoice, float volume)
{
	// -------------------------------------------------------
	...
	status = pVoice->Start(volume);
	assert(status == Handle::Status::SUCCESS);
}

void AudioEngine::PanSound(Voice* pVoice, float snd_pan)
{
	// -------------------------------------------------------
	// Start the source voice
	// -------------------------------------------------------
	assert(pVoice);
	Handle::Status status;
	status = pVoice->Pan(this->poMasterVoice, snd_pan);
	assert(status == Handle::Status::SUCCESS);
}
