...
Handle::Status Snd::Play(float volume)
{
...
Handle::Status Snd::Stop()
{
	...
	if (lock)
	{
		Stop_ASndCmd* pCmd = new Stop_ASndCmd(this->snd_id, this);
		assert(pCmd);
		SndMan::Send(pCmd);
	}
...
Handle::Status Snd::SetVolume(float volume)
{
	Handle::Lock lock(this->handle);

	if (lock)
	{
		Volume_ASndCmd* pCmd = new Volume_ASndCmd(this->snd_id, this);
		assert(pCmd);
		pCmd->SetVolume(volume);
		SndMan::Send(pCmd);
	}
...
Handle::Status Snd::Pan(float snd_pan)
{
	...
	if (lock)
	{
		Pan_ASndCmd* pCmd = new Pan_ASndCmd(this->snd_id, this);
		pCmd->SetPan(snd_pan);
		assert(pCmd);
		SndMan::Send(pCmd);
	}
